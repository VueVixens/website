  ...
  <v-layout class="vv-container" fill-height>
    <template v-if="!genderQuestionAnswered && !showSorryMessage">
      <v-container fill-height class="raised">
        ...
        <div class="actions">
          ...
          >{{$t('genderQuestion.yes')}}</v-btn>
          <v-btn
            color="secondary darken-2"
            dark
            @click="proceedToSorryMessage"
          >{{$t('genderQuestion.no')}}</v-btn>
        </div>
      </v-container>
    </template>
    <template v-else-if="!showSorryMessage">
      <v-container class="indexed">
        <v-flex xs12>
          <h1 class="display-2 text-xs-center">{{$t('heading')}}</h1>
          <v-form
            ...
            v-model="valid"
          >
            <v-text-field name="EMAIL" :label="$t('email')" v-model="email" :rules="emailRules"></v-text-field>
            ...
            <p class="text-xs-left caption">
              ...
              <a
                ...
                target="_blank"
              >{{$t('content.question')}}</a>
            </p>
            <v-btn color="secondary darken-2" dark type="submit" :disabled="!valid">{{$t('submit')}}</v-btn>
          </v-form>
        </v-flex>
      </v-container>
    </template>
